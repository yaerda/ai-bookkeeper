...
- `GET /api/categories?ledgerId=...`：读取账本共享的分类名称、图标、颜色和排序。
- `POST /api/categories?ledgerId=...`：为账本新增分类；按类型和名称幂等。
- `POST /api/categories/import?ledgerId=...`：合并本地分类，每批最多 200 条，不覆盖云端已有定义。
- `GET/PATCH /api/privacy/settings`：读取或更新当前账号的 Web 隐私口令要求。
- `POST /api/privacy/verify`：服务端校验口令及设置版本。
- `POST /api/privacy/migrate`：将尚未配置云端口令的账号从旧版本机校验值迁移。
...
`ledger_category` 使用独立云端 ID，并以 `(ledger_id, type, name)` 去重。同一家庭账本的
所有成员读取同一个目录；`OWNER`、`EDITOR` 可新增，`VIEWER` 只能读取。分类不随邀请
复制到成员个人账本，切换账本时也不复用上一账本的目录。默认分类与 Android 的 16 项一致，
迁移保留历史交易中的有效自定义名称；已有交易的分类快照不会被改写。

Android 默认本人账本保留离线 Room 分类和原有本地 ID，云同步时导入尚未上传的分类，
再将云端目录按名称和收支类型映射到本地。其他本人账本和受邀账本直接使用各自的云端目录。
Web 新增、编辑和分类管理使用该目录，保存交易时同时写入所选名称、图标和颜色。

### Web 口令随账号同步

`user_privacy` 绑定经验证的 `app_user.id`，不绑定邮箱、浏览器或账本。家庭共享不会共享
任何成员的个人口令。服务器仅保存加盐校验值、开关、版本及失败次数；客户端设置响应不包含
校验值。新口令使用 PBKDF2-SHA256 600,000 次迭代，旧浏览器的 120,000 次迭代校验值仅能
初始化尚无云端配置的账号。明确清除口令仍保留账号设置行，防止旧设备重新覆盖。

Web 在确认云端口令要求前不展示账本，配置读取失败时保持阻断；其他设备更改版本后重新解锁。
修改、关闭或验证口令都由服务端处理，连续五次错误会锁定五分钟。口令是 Web 附加隐私锁，
不改变 Entra 认证、Android 登录或交易 API 的授权模型。
